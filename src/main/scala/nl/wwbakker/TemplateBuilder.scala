...
object TemplateBuilder {
  ...
  def elasticSearchDomain(configuration: DeploymentConfiguration) = `AWS::Elasticsearch::Domain`(
  ...
  def outputs : Seq[Output[String]] = Seq(
    Output(
      name = "DomainArn",
  ...
  def template(configuration: DeploymentConfiguration): Template = Template(
    Description = Some(configuration.stackName),
